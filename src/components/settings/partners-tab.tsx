...
import {
  partners, type Partner, type PartnerStatus,
} from "@/lib/mock-data"
      ...
      <CardContent className="p-4">
        <div className="flex items-start gap-3">
          <div className={`size-10 rounded-lg flex items-center justify-center text-lg shrink-0 ${
            isAvailable ? "bg-emerald-100 dark:bg-emerald-900/30" : "bg-amber-100 dark:bg-amber-900/30"
          }`}>
            {adapter.icon}
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              ...
              <span className={`inline-flex items-center gap-1 rounded-md border px-2 py-0.5 text-xs font-medium shrink-0 ${
                ...
                {isAvailable ? (
                  <><CheckCircle className="size-3" />Disponible</>
                ) : (
...
function PartnerDetailDialog({ partner, open, onOpenChange }: {
  partner: Partner | null; open: boolean; onOpenChange: (open: boolean) => void
}) {
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="mt-1"><PartnerStatusBadge status={partner.statut} /></div>
            </div>
          </div>
          <Separator />
...
function PartnerCard({ partner, onViewDetail }: { partner: Partner; onViewDetail: (p: Partner) => void }) {
  return (
      ...
      <CardContent className="p-4">
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" className="flex-1 text-xs h-7" onClick={() => onViewDetail(partner)}>
              <Eye className="size-3 mr-1" />Détails
            </Button>
            <Button variant="outline" size="sm" className="flex-1 text-xs h-7">
              <Pencil className="size-3 mr-1" />Configurer
            </Button>
...
export function PartnersTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      {filteredPartners.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filteredPartners.map((partner) => (
            <PartnerCard key={partner.id} partner={partner} onViewDetail={openDetail} />
          ))}
        </div>
      )}
      ...
      {/* Partner Detail Dialog */}
      <PartnerDetailDialog partner={selectedPartner} open={detailOpen} onOpenChange={setDetailOpen} />
    </div>
  )
}
